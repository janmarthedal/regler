...
/// the corresponding replacement term, recursing into application arguments and
/// the application head.
pub fn subst(t: &Term, sigma: &HashMap<Symbol, Term>) -> Term {
    match t {
        Term::Nat(_) | Term::Int(_) | Term::Rat(_) | Term::Sym(_) => t.clone(),
        Term::Var(s) => match sigma.get(s) {
            Some(replacement) => replacement.clone(),
            None => t.clone(),
        },
        Term::App(head, args) => {
            let new_head = subst(head, sigma);
